...
export const load = (async ({ params, locals }) => {
    const { variant_supply_id } = params
    return {
        variant_supply: await (await locals.supabaseClient.from('variant_supply').select('*,supplies(name,unit)').eq("id", variant_supply_id).limit(1).single()).data
    };
